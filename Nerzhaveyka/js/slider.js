/**
 * slider.js — Нержавейка Майкоп
 * Полная логика Hero-слайдера
 *
 * Функционал:
 *  - Переключение слайдов через CSS transform (GPU-анимация)
 *  - Активные классы по БЭМ: slider__slide_active, slider__dot_active
 *  - Auto-play с таймером
 *  - Hover-pause (остановка при наведении на слайдер)
 *  - Кнопки Prev / Next
 *  - Навигационные точки (dots)
 *  - Progress-bar (визуализация таймера)
 *  - Корректный пересчёт размеров при ресайзе (ResizeObserver)
 *  - Touch / Swipe поддержка на мобильных
 *  - Keyboard (стрелки ← →) при фокусе на слайдере
 *  - Пауза при потере видимости вкладки (Page Visibility API)
 *
...
class HeroSlider {
  /**
   * @param {string} rootSelector — CSS-селектор корневого элемента .slider
   * @param {object} config
   */
  constructor(rootSelector, config = {}) {
    ...
    this.isPaused   = false;
    this.timer      = null;
    this.isAnimating = false; // защита от двойного клика
  ...
  /**
   * Переключиться на нужный индекс
   * @param {number} index — целевой слайд
   * @param {boolean} [animate=true] — с CSS-переходом или мгновенно
   */
  _setSlide(index, animate = true) {
    ...
    /* --- Wrapper transform --- */
    if (!animate) {
      // Отключаем transition на время первой установки
    ...
    /* --- aria-live для скрин-ридеров --- */
    this.slides.forEach((slide, i) => {
      slide.setAttribute('aria-hidden', i !== this.current ? 'true' : 'false');
    });

    /* --- Разблокировка после окончания перехода --- */
    if (animate) {
      setTimeout(() => {
        this.isAnimating = false;
      }, this.cfg.transitionMs + 50);
    } else {
      // Возвращаем transition на следующем кадре (обход layout)
    ...
    /* --- Сброс и перезапуск progress-bar --- */
  ...
  _bindControls() {
    this.btnPrev?.addEventListener('click', () => {
      this.prev();
      // Перезапуск таймера после ручного переключения
      if (!this.isPaused) {
        clearTimeout(this.timer);
        this._scheduleNext();
      }
  ...
  _bindTouch() {
    ...
    this.root.addEventListener('touchmove', e => {
      ...
      // Определяем направление первого движения: горизонталь vs вертикаль
      const dx = Math.abs(e.touches[0].clientX - this._touchStartX);
      const dy = Math.abs(e.touches[0].clientY - this._touchStartY);
      // Если скролл преобладает — прекращаем отслеживание
  ...
  /** Клавиатура (стрелки) при фокусе внутри слайдера */
  _bindKeyboard() {
  ...
  /** Page Visibility API — пауза на скрытой вкладке */
  _bindVisibility() {
  ...
  /**
   * ResizeObserver — пересчёт позиции при изменении размеров
   * Без него translateX(-N*100%) сломается при смене ширины контейнера
   */
  _bindResize() {
  ...
  _onResize() {
    // Мгновенно (без анимации) переставить wrapper на текущий слайд
    this.wrapper.style.transition = 'none';
    this.wrapper.style.transform  = `translateX(-${this.current * 100}%)`;
...
document.addEventListener('DOMContentLoaded', () => {
  // Создаём инстанс слайдера и делаем его доступным глобально
  // (для возможной интеграции с другими скриптами)
